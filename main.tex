...
\section{Measurements Setup}
    ...
    L'applicazione delle tecniche di computer vision richiede di introdurre un modello matematico con il quale descrivere le telcamere. Il modello di telecamera definisce la relazione matematica necessaria a legare le coordinate del mondo fisico ($m$) a quelle dei pixel delle immagini ($M$).  
    In questo lavoro il modello matematico scelto è il pinhole camera model desctitto dall'Equazione~\ref{eq:pinhole}. 
    ...
    In questa relazione i termini $R$ e $T$ sono noti come parametri estrinseci e rappresentano rispettivamente la matrice di rotazione e il vettore di traslazione che legano le coordinate del mondo fisico con il sistema di riferimento della telecamera. La matrice $K$ prende il nome di matrice della telecamera e contiene i parametri intrinseci del modello. Nello specifico la matrice $K$ assume la forma mostrata dall'Equazione~\ref{eq:cameraMat}.
    ...
    Nella quale i termini $f_x$ e $f_y$ costituiscono le lunghezze focali della telecamera lungo i due assi, mentre $c_x$ e $c_y$ sono le coordinate in pixel del punto principale della telecamera.

    Questo modello di telecamera offre numerosi vantaggi tra cui una notevole semplicità d'uso e una buona accuratezza e affidabilità.

    Tuttavia, il modello pinhole, specie utilizzato in un ambiente non convenzionale come un tunnel di cavtazione, presenta dei limiti. Il modello pinhole si basa sull'ipotesi che i raggi prospettici siano rettilieni. Nel presente caso questa ipotesi è ovviamente falsa a causa della rifrazione della luce causata dell'interfaccia aria-plexiglass-acqua presente nel tunnel. 

    Inoltre il modello pinhole descrive una telecamera priva di lenti. Nelle applicazioni pratiche le lenti sono necessarie per ottenere immagini di qualità. However, lenses, which are essential in practical applications, introduce optical distortions that can affect measurements. Oltre alle lenti, per queste applicazioni, è spesso conveniente aggiungere dispositivi Scheimpflug al setup. Questi dispositivi migliorano la profondità di campo inclinando il piano focale, permettendo a elementi vicini e lontani di rimanere a fuoco. Tuttavia, i sistemi Scheimpflug introducono distorsioni ottiche nelle immagini acquisite.

    Adottare soluzioni adeguate per mitigare questi problemi è essenziale per garantire la qualità delle misure. In letteratura si trovano diversi modelli matematici in grado di descrivere e correggere le principali sorgenti di distorsione. In questo lavoro questi modelli sono stati combinati in nove differenti configurazioni al fine di scegliere quella più appropriata al caso specifico.
